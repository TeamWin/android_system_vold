...
bool retrieveKey(const std::string& dir, const KeyAuthentication& auth, KeyBuffer* key) {
    ...
    LOG(INFO) << "retrieveKey::usesKeymaster::1";
    if (auth.usesKeymaster()) {
        LOG(INFO) << "retrieveKey::usesKeymaster::2";
        Keymaster keymaster;
        LOG(INFO) << "retrieveKey::usesKeymaster::3";
        if (!keymaster) return false;
        LOG(INFO) << "retrieveKey::usesKeymaster::4";
        km::AuthorizationSet keyParams = beginParams(appId);
        LOG(INFO) << "retrieveKey::usesKeymaster::5";
        if (!decryptWithKeymasterKey(keymaster, dir, keyParams, encryptedMessage, key))
            return false;
        LOG(INFO) << "retrieveKey::usesKeymaster::6";
    } else {
        if (!decryptWithoutKeymaster(appId, encryptedMessage, key)) return false;
    }
    LOG(INFO) << "retrieveKey::usesKeymaster::7";
    return true;
}
