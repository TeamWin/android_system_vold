...
extern "C" uint_8t* get_policy_descriptor(fscrypt_policy* fep) {
	if (!fep) return NULL;
	switch(fep->version) {
...
extern "C" uint_8t get_policy_size(fscrypt_policy* fep, bool hex) {
	if (!fep) return 0;
	switch(fep->version) {
		case FSCRYPT_POLICY_V1:
...
char* get_policy_content(fscrypt_policy* fep) {
	if (!fep) return NULL;
	char *content = NULL;
	switch(fep->version) {
		case FSCRYPT_POLICY_V1:
			content = (char*)malloc(sizeof (struct fscrypt_policy_v1));
			...
			break;
		case FSCRYPT_POLICY_V2:
			content = (char*)malloc(sizeof (struct fscrypt_policy_v2));
