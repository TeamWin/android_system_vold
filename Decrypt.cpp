			...
			* https://android.googlesource.com/platform/frameworks/base/+/android-8.0.0_r23/keystore/java/android/security/keystore/AndroidKeyStoreAuthenticatedAESCipherSpi.java#216 */
			...
			if (!rc.isOk()) {
				...
				if (ks2::ResponseCode(error) == ks2::ResponseCode::KEY_NOT_FOUND) {
					...
					::ndk::SpAIBinder keystoreLegacyBinder(AServiceManager_checkService("android.security.legacykeystore"));
					auto keystoreLegacy = ks2::IKeystoreService::fromBinder(keystoreLegacyBinder);
					rc = keystoreLegacy->getKeyEntry(keyDescriptor(keystore_alias), &keyEntryResponse);

					if (!rc.isOk()) {
						std::shared_ptr<ks2::IKeystoreSecurityLevel> sec_level;
						auto ret = keystoreLegacy->getSecurityLevel(keymint::SecurityLevel::TRUSTED_ENVIRONMENT, &sec_level);
