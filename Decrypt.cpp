...
static bool lookup_ref_key_internal(std::map<userid_t, android::fscrypt::EncryptionPolicy> key_map, const uint8_t* policy, __u8 size, __u8 hex_size, userid_t* user_id) {
...
extern "C" bool lookup_ref_key(fscrypt_policy* fep, uint8_t* policy_type) {
	...
	__u8 *descryptor = get_policy_descriptor(fep);
	__u8 hex_size = get_policy_size(fep, true);
	__u8 size = get_policy_size(fep, false);
	char policy_hex[hex_size];
	bytes_to_hex(descryptor, size, policy_hex);
	if (std::strncmp((const char*)descryptor, de_key_raw_ref.c_str(), size) == 0) {
	...
	if (!lookup_ref_key_internal(s_de_policies, descryptor, size, hex_size, &user_id)) {
		if (!lookup_ref_key_internal(s_ce_policies, descryptor, size, hex_size, &user_id)) return false;
