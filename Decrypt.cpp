...
static bool lookup_ref_key_internal(std::map<userid_t, android::fscrypt::EncryptionPolicy> key_map, const uint8_t* policy, uint_8t size, uint_8t hex_size, userid_t* user_id) {
...
extern "C" bool lookup_ref_key(fscrypt_policy* fep, uint8_t* policy_type) {
	...
	uint_8t *descriptor = get_policy_descriptor(fep);
	uint_8t hex_size = get_policy_size(fep, true);
	uint_8t size = get_policy_size(fep, false);
