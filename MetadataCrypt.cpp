...
bool fscrypt_mount_metadata_encrypted(const std::string& blk_device, const std::string& mount_point,
                                      ...
                                      const std::string& fs_type, fs_mgr::Fstab *fstab) {
    ...
    if (version == 1) {
        // Force Unset Props for V1
        property_override("ro.crypto.volume.metadata.method","");
        property_override("ro.crypto.dm_default_key.options_format.version","1");
        property_override("ro.crypto.volume.options","");
        ...
        property_override("ro.crypto.volume.options","::v2");
        if (!parse_options(data_rec->metadata_encryption, &options)) return false;
    } else {
        LOG(ERROR) << "Unknown options_format_version: " << version;
        return false;
    }
